...
`hermas_control_submit` accepts exactly one complete, validated `EXECUTE`
...
`hermas_control_collect` exposes only a terminal daemon result and encodes it
...
`hermas_control_server` owns at most 16 caller connections, matching the
