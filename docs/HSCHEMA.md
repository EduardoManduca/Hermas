...
edge requires the same catalog type ID. HScript will require an explicit
...
HSchema does not claim literal reversibility, prove business-level recovery,
or infer an undo operation. The app developer owns those semantics; HScript
decides whether a workflow enrolls the capability in an explicit saga.

Delivery uncertainty is not an HSchema app-error type. Every invocation
acquires a separate delivery-`Unknown` graph port from the execution model.

## Schema and Action fingerprints

`hermas` emits two related identities:

- The schema fingerprint covers the complete app file through the versioned
  `hermas-contract-v1` canonical form. It is useful for catalog inspection.
- Each Action fingerprint covers one Action's name, ports, compensation
  ...
  declarations through `hermas-action-v1`. Graph images and daemon
...
trust. Hermas has no cryptographic key dependency.
...
```text
contract.hschema:7:18: schema/unknown-type: unknown type `Missing`
```

The three executable proof contracts live in
`apps/grade-pipeline/*.hschema`. They are ordinary external inputs loaded
