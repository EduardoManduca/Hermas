...
HScript2 is notation for constructing verified Action graphs. One source file
...
remains one `.h2gi` graph image per selected workflow; the compiler never
...
type. HSchema2 recursive representation compatibility must also prove that
...
`NotSent` and `Unknown` are protocol states, not HSchema2 types, and require no
...
```text
herma2 workflow check MODULE SCHEMA...
herma2 workflow explain [--workflow NAME] MODULE SCHEMA...
herma2 workflow graph [--workflow NAME] MODULE SCHEMA...
herma2 workflow resources [--workflow NAME] MODULE SCHEMA...
herma2 workflow sources [--workflow NAME] MODULE SCHEMA...
herma2 workflow image [--workflow NAME] MODULE OUTPUT.h2gi SCHEMA...
```
