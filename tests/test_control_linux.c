...
static int submit(
    ...
    return hermas2_protocol_encode(
               &request, packet, HERMAS2_PROTOCOL_MAX_PACKET_SIZE,
               packet_size) == HERMAS2_PROTOCOL_OK &&
           hermas2_control_submit(
...
int main(int argc, char **argv) {
    ...
    if (hermas2_control_submit(
        ...
        hermas2_control_collect(
            loop, 100u, packet, HERMAS2_PROTOCOL_MAX_PACKET_SIZE,
            &packet_size) != HERMAS2_CONTROL_EXECUTION_ACTIVE ||
        hermas2_control_release(loop, 100u) !=
    ...
    if (submit(
            loop, input_type,
            100u + HERMAS2_DAEMON_MAX_EXECUTIONS,
            packet, &packet_size)) {
