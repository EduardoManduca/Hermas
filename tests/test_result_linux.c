...
int main(void) {
    char path[] = "/tmp/hermas2-result-XXXXXX";
    ...
    if (hermas2_result_file_open(&file, path, &summary) !=
    ...
    if (descriptor < 0 || write(descriptor, &byte, 1u) != 1 ||
        close(descriptor) != 0 ||
        hermas2_result_file_open(&file, path, &summary) ==
            HERMAS2_RESULT_STORE_OK) {
        hermas2_result_file_close(&file);
        unlink(path);
