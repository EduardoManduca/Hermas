...
int main(int argc, char **argv) {
    ...
    hermas_control_server *server = malloc(sizeof(*server));
    if (loop == NULL || server == NULL ||
        ...
        hermas_control_server_init(server, loop) !=
            HERMAS_CONTROL_SERVER_OK) {
        return fail("cannot initialize server fixture");
    }
    int valid[2];
    if (socketpair(AF_UNIX, SOCK_SEQPACKET, 0, valid) != 0 ||
        hermas_control_server_attach(server, valid[0]) !=
            HERMAS_CONTROL_SERVER_OK ||
        !send_execute(valid[1], read_u16(image, 22u), 71u)) {
        return fail("cannot attach valid caller");
    }
    size_t progress = 0u;
    if (hermas_control_server_step(server, 0, &progress) !=
            HERMAS_CONTROL_SERVER_OK ||
        hermas_daemon_loop_active(loop) != 1u ||
        hermas_control_server_active(server) != 1u) {
        return fail("valid caller was not admitted");
    }
    close(valid[1]);
    if (hermas_control_server_step(server, 10, &progress) !=
            HERMAS_CONTROL_SERVER_OK ||
        hermas_daemon_loop_active(loop) != 1u ||
        hermas_control_server_active(server) != 1u ||
    ...
    if (socketpair(AF_UNIX, SOCK_SEQPACKET, 0, invalid) != 0 ||
        hermas_control_server_attach(server, invalid[0]) !=
            HERMAS_CONTROL_SERVER_OK) {
        return fail("cannot attach invalid caller");
    }
    uint8_t malformed = 0u;
    if (send(invalid[1], &malformed, 1u, 0) != 1 ||
        hermas_control_server_step(server, 10, &progress) !=
            HERMAS_CONTROL_SERVER_OK) {
        return fail("cannot process invalid caller");
    }
    uint8_t response[HERMAS_PROTOCOL_MAX_PACKET_SIZE];
    ...
    if (received <= 0 ||
        hermas_protocol_decode(
            ...
            HERMAS_PROTOCOL_OK ||
        error.kind != HERMAS_FRAME_PROTOCOL_ERROR ||
        hermas_control_server_active(server) != 1u) {
        return fail("invalid caller was not rejected");
    }
    close(invalid[1]);
    hermas_control_server_close(server);
    ...
    if (hermas_control_server_attach(server, caller[0]) !=
            HERMAS_CONTROL_SERVER_OK ||
    ...
    uint8_t invocation_packet[HERMAS_PROTOCOL_MAX_PACKET_SIZE];
    ...
    if (hermas_protocol_encode(
            ...
            &app_error_size) != HERMAS_PROTOCOL_OK ||
    ...
    hermas_frame execution_result;
    if (received <= 0 ||
        hermas_protocol_decode(
            response, (size_t)received, &execution_result) !=
            HERMAS_PROTOCOL_OK ||
        execution_result.kind != HERMAS_FRAME_EXECUTION_RESULT ||
        execution_result.outcome != HERMAS_OUTCOME_APP_ERROR ||
        hermas_control_server_active(server) != 0u ||
