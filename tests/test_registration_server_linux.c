...
int main(int argc, char **argv) {
    ...
    if (image == NULL ||
        hermas_image_validate(image, image_size, &summary) !=
            HERMAS_IMAGE_OK ||
    ...
    if (hermas_daemon_registry_init(
        ...
        hermas_registration_server_init(
            &server, &registry) != HERMAS_REGISTRATION_SERVER_OK) {
        free(image);
    ...
    if (socketpair(AF_UNIX, SOCK_SEQPACKET, 0, slow) != 0 ||
        ...
        hermas_registration_server_attach(&server, slow[1]) !=
            HERMAS_REGISTRATION_SERVER_OK ||
        hermas_registration_server_attach(&server, ready[1]) !=
            HERMAS_REGISTRATION_SERVER_OK) {
        return fail("could not attach pending connections");
    }
    size_t progress = 99u;
    if (hermas_registration_server_step(&server, 0, &progress) !=
            HERMAS_REGISTRATION_SERVER_OK ||
        progress != 0u ||
        hermas_registration_server_pending(&server) != 2u) {
    ...
    if (!send_registration(
        ...
        hermas_registration_server_step(&server, 0, &progress) !=
            HERMAS_REGISTRATION_SERVER_OK ||
        progress != 2u ||
        hermas_registration_server_pending(&server) != 1u ||
        ...
        !receive_kind(ready[0], HERMAS_FRAME_REGISTER_OK)) {
    ...
    if (socketpair(AF_UNIX, SOCK_SEQPACKET, 0, wrong) != 0 ||
        hermas_registration_server_attach(&server, wrong[1]) !=
            HERMAS_REGISTRATION_SERVER_OK ||
        !send_registration(
        ...
        !receive_kind(wrong[0], HERMAS_FRAME_PROTOCOL_ERROR)) {
    ...
    if (socketpair(AF_UNIX, SOCK_SEQPACKET, 0, duplicate) != 0 ||
        hermas_registration_server_attach(&server, duplicate[1]) !=
            HERMAS_REGISTRATION_SERVER_OK ||
        !send_registration(
        ...
        progress != 1u ||
        !receive_kind(
            duplicate[0], HERMAS_FRAME_PROTOCOL_ERROR)) {
    ...
    if (socketpair(AF_UNIX, SOCK_SEQPACKET, 0, excess) != 0 ||
        hermas_registration_server_attach(&server, excess[1]) !=
            HERMAS_REGISTRATION_SERVER_CAPACITY_EXHAUSTED ||
        hermas_registration_server_pending(&server) !=
            HERMAS_REGISTRATION_MAX_PENDING) {
        return fail("registration capacity was not enforced");
    }
    close(excess[0]);
    close(excess[1]);

    hermas_registration_server_close(&server);
    hermas_registration_server_close(&server);
    for (size_t index = 0u;
         index < HERMAS_REGISTRATION_MAX_PENDING; ++index) {
