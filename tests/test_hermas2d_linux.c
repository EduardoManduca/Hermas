...
static int fail(const char *message) {
    fprintf(stderr, "test_hermas2d: %s\n", message);
    return 1;
}
...
static int run_cycle(
    ...
    if (!use_runner &&
        hermas2_client_connect(&client, control_socket) !=
            HERMAS2_CLIENT_OK) {
...
static void remove_state(const char *directory) {
    static const char *const files[] = {
        "journal.h2j", "results.h2r",
        "compensation.h2c", "saga.h2s"
    };
    char path[512];
    for (size_t index = 0u;
...
int main(int argc, char **argv) {
    ...
    char secure_image[] = "/tmp/hermas2d-image-XXXXXX";
    ...
    char state[] = "/tmp/hermas2d-state-XXXXXX";
    ...
    char recovery[] = "/tmp/hermas2d-recovery-XXXXXX";
    ...
    if (hermas2_journal_file_open(
            &journal, journal_path, &summary) != HERMAS2_JOURNAL_OK ||
        hermas2_journal_writer_append(
            &journal.writer, started) != HERMAS2_JOURNAL_OK) {
    ...
    unlink(secure_image);
    free(image);
    puts("runnable hermas2d host tests passed");
    return 0;
}
