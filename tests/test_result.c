...
static hermas_result_record result_record(
    uint64_t sequence,
    const uint8_t *value,
    uint32_t value_length) {
    return (hermas_result_record){
        .sequence = sequence,
        .key = {
            .execution_id = 19u,
            .workflow_id = 7u,
...
static void test_codec_scan_and_lookup(void) {
    ...
    uint8_t bytes[2u * (HERMAS_RESULT_HEADER_SIZE + 8u)];
    size_t size = 0u;
    require(hermas_result_encode(
                &input, bytes, sizeof(bytes), &size) ==
                HERMAS_RESULT_STORE_OK &&
                size == HERMAS_RESULT_HEADER_SIZE + sizeof(value) &&
                memcmp(bytes, "HRS1", 4u) == 0,
            "result record did not encode");
    hermas_result_record decoded;
    size_t decoded_size = 0u;
    require(hermas_result_decode(
                bytes, size, &decoded, &decoded_size) ==
                HERMAS_RESULT_STORE_OK &&
                decoded_size == size &&
                decoded.outcome == HERMAS_OUTCOME_APP_ERROR &&
    ...
    uint8_t output[8];
    int found = 0;
    hermas_result_record match;
    require(hermas_result_find(
    ...
    bytes[HERMAS_RESULT_HEADER_SIZE] ^= 1u;
    require(hermas_result_decode(
                bytes, size, &decoded, &decoded_size) ==
...
typedef struct memory_sink {
    uint8_t bytes[HERMAS_RESULT_HEADER_SIZE + 8u];
    size_t length;
} memory_sink;

static hermas_result_store_result memory_write(
    void *context,
...
static void test_writer(void) {
    ...
    uint8_t value[8] = {9u};
    uint8_t scratch[HERMAS_RESULT_HEADER_SIZE + 8u];
    hermas_result_record record =
        result_record(0u, value, sizeof(value));
    require(hermas_result_writer_init(
    ...
    require(hermas_result_writer_append(
                &writer, record, scratch, sizeof(scratch)) ==
