...
int main(int argc, char **argv) {
    if (argc == 2 && strcmp(argv[1], "--version") == 0) {
        printf(
            "Hermas %s (hermas_image_check; graph-image %u)\n",
            HERMAS_VERSION, HERMAS_GRAPH_IMAGE_VERSION);
        return 0;
    }
    if (argc == 2 && strcmp(argv[1], "--help") == 0) {
        puts(
            "usage: hermas_image_check [--describe] IMAGE\n\n"
    ...
    hermas_image_result result = hermas_image_validate(
        bytes, (size_t)length, &summary);
    if (result != HERMAS_IMAGE_OK) {
        fprintf(stderr, "%s\n", hermas_image_result_name(result));
        free(bytes);
        return 1;
    }
    if (describe) {
        ...
        size_t contracts = read_u32(
            bytes,
            HERMAS_IMAGE_HEADER_ACTION_CONTRACTS_OFFSET);
