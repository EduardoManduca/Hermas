...
Hermas exists to let users construct and execute typed action graphs between
...
> dependencies across multiple Actions, it may belong in Hermas.
...
HScript constructs relationships between Actions:
...
HScript does not calculate values. Arithmetic, filtering, transformation,
policy, formatting, parsing, and domain decisions are application behaviors.

This makes HScript small while allowing the Hermas ecosystem to become broad.
...
Hermas users should be able to inspect the exact verified graph produced by
their source:
...
Those capabilities remain composable through Hermas without increasing the
complexity or privilege of `hermasd`.
...
Hermas has no obligation to preserve the legacy prototype's grammar, image,
...
HSchema declares their typed ports.
