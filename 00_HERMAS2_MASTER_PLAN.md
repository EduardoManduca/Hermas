# Hermas2 Master Architecture Plan

## 1. Project Charter

Hermas2 is an experimental, clean-slate action protocol and orchestration
system for independent applications on one Linux host.

Applications publish typed semantic operations called **Actions**. HScript2
users construct verified action graphs from those operations. The Hermas2
...
Hermas2 is not currently a replacement for Hermas. It is an independent
architecture experiment. No source, image, ABI, or wire compatibility is
required while both projects remain prototypes.

## 2. Why Hermas2 Exists

The current Hermas implementation demonstrates that the basic philosophy
works:
...
Hermas2 explores what happens when that graph is the language's public semantic
model from the beginning instead of primarily a compact runtime encoding.

## 3. Compatibility Position

Hermas2 may reuse good ideas, but it does not preserve compatibility merely
...
The existing Hermas implementation remains a valuable comparison oracle.
Equivalent scenarios should be compared when useful, but equivalence is an
experiment—not an obligation.

## 4. Core Mental Model

Hermas2 has five primary objects:

1. **HSchema2 contracts** define app-owned nominal values and Action ports.
2. **HScript2 expressions** construct typed graph fragments.
...
```text
Apps declare possible Action nodes through HSchema2
                         |
                         v
HScript2 composes typed graph fragments
                         |
                         v
...
- Execution history needed to explain what Hermas2 did.
...
HScript2 is a graph-construction DSL.

An HScript2 source file is a module and may declare multiple workflows.
...
### 6.3 `herma2`: Rust Frontend

The Rust frontend owns:

- HSchema2 and HScript2 parsing.
- HScript2 module parsing and unique workflow-name resolution.
- Source diagnostics.
...
It does not parse HScript2, resolve source names, build graphs, supervise apps,
...
An HScript2 expression constructs a graph fragment. A fragment exposes:
...
`Unknown` means complete delivery may have occurred but Hermas2 cannot know
the app's final side-effect state.

Initial HScript2 rules:
...
- Hermas2 never retries automatically.
- Hermas2 never runs compensation after `Unknown` without an explicit,
  separately completed reconciliation Action.

This keeps uncertainty outside app-owned domain variants.

## 10. HScript2 Expression Direction
...
```text
herma2 check module.hscript2
herma2 graph module.hscript2 WORKFLOW
herma2 explain module.hscript2 WORKFLOW
herma2 resources module.hscript2 WORKFLOW
herma2 compile module.hscript2 WORKFLOW
```
...
not separately interpreted from HScript2.
...
representation. Hermas2 does not turn the delivery journal into an
application-data store.

## 16. Ecosystem Model

Third-party capabilities are ordinary isolated apps exposing HSchema2
contracts.
...
Implement without HScript2 parsing:
...
- HSchema2 can fully describe the three Grade Pipeline apps without runtime
  application code.

### Milestone 3: Sequential HScript2
...
This extension is not part of the essential Hermas2 implementation. It is
...
Hermas2 is successful only if the graph-first model proves itself in code.

Compare it with the original Hermas on:
...
- HScript2 readability.
- Throughput and latency.

Hermas2 is allowed to be incompatible. It is not allowed to be sophisticated
...
- Final HSchema2 and HScript2 file extensions.
...
A contributor should be able to explain Hermas2 in one sequence:

```text
HSchema2 defines typed Action ports.
HScript2 connects those ports into readable expressions.
