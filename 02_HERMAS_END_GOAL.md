...
Status: product vision for the independent Hermas experiment.

## 1. Vision

The long-term vision is:

> Hermas is a local, decentralized action protocol and orchestration system
...
Hermas introduces a shared semantic boundary. Instead of saying:
...
1. Describe the app's semantic types in HSchema.
...
- How Hermas users organize source or graph-image files.
- Which user interface initiated the Action.

The app publishes capabilities, not predefined integrations.

## 4. The HScript User Experience

A Hermas user should primarily need HScript and local discovery tools.
...
- Launch or supervise production apps through HScript.
...
- An application that generates HScript.
...
Hermas does not embed user-interface behavior in the daemon.

A graphical editor is therefore a client of Hermas, not part of Hermas's execution semantics.

## 7. The Application Ecosystem

Hermas remains useful by allowing external software to add capabilities as
Actions.
...
> Hermas provides the rules for constructing verified action graphs. The
> ecosystem provides the Actions worth placing in those graphs.

## 8. Decentralization

Hermas is decentralized at the application level:
...
`hermasd` is a local coordinator, not a central owner of application behavior
or domain truth.
...
Hermas reaches its intended end state when:

- Applications can expose semantic operations with little integration code.
- Users compose useful behavior primarily through HScript.
...
- Adding new capabilities normally means adding an app, not expanding HScript.
...
## 10. What Hermas Is Not Trying to Become
