...
typedef struct scan_execution {
    hermas2_journal_interrupted value;
...
static bool is_action_record(hermas2_journal_kind kind) {
    return kind >= HERMAS2_JOURNAL_DELIVERY_PREPARED &&
           kind <= HERMAS2_JOURNAL_ACTION_UNKNOWN;
}

static hermas2_journal_result validate_record(
    const hermas2_journal_record *record) {
    if (record->kind < HERMAS2_JOURNAL_EXECUTION_STARTED ||
        record->kind > HERMAS2_JOURNAL_EXECUTION_FINISHED) {
        return HERMAS2_JOURNAL_INVALID_KIND;
    }
    bool outcome_valid = false;
    switch (record->kind) {
        case HERMAS2_JOURNAL_EXECUTION_STARTED:
        case HERMAS2_JOURNAL_DELIVERY_PREPARED:
        case HERMAS2_JOURNAL_DELIVERY_SENT:
            outcome_valid = record->outcome == HERMAS2_OUTCOME_NONE;
            break;
        case HERMAS2_JOURNAL_ACTION_SUCCEEDED:
            outcome_valid = record->outcome == HERMAS2_OUTCOME_SUCCESS;
            break;
        case HERMAS2_JOURNAL_ACTION_FAILED:
            outcome_valid =
                record->outcome == HERMAS2_OUTCOME_APP_ERROR ||
                record->outcome == HERMAS2_OUTCOME_NOT_SENT;
            break;
        case HERMAS2_JOURNAL_ACTION_UNKNOWN:
            outcome_valid = record->outcome == HERMAS2_OUTCOME_UNKNOWN;
            break;
        case HERMAS2_JOURNAL_EXECUTION_FINISHED:
            outcome_valid =
...
hermas2_journal_result hermas2_journal_encode(
    ...
    if (destination_size < HERMAS2_JOURNAL_RECORD_SIZE) {
        return HERMAS2_JOURNAL_BUFFER_TOO_SMALL;
    }
    hermas2_journal_result valid = validate_record(record);
    if (valid != HERMAS2_JOURNAL_OK) {
        return valid;
    }
    memset(destination, 0, HERMAS2_JOURNAL_RECORD_SIZE);
    memcpy(destination, "H2JR", 4u);
    write_u16(destination, 4u, HERMAS2_JOURNAL_VERSION);
    write_u16(destination, 6u, HERMAS2_JOURNAL_RECORD_SIZE);
...
hermas2_journal_result hermas2_journal_decode(
    ...
    if (source_size < HERMAS2_JOURNAL_RECORD_SIZE) {
        return HERMAS2_JOURNAL_BUFFER_TOO_SMALL;
    }
    if (memcmp(source, "H2JR", 4u) != 0) {
        return HERMAS2_JOURNAL_INVALID_MAGIC;
    }
    if (read_u16(source, 4u) != HERMAS2_JOURNAL_VERSION) {
        return HERMAS2_JOURNAL_INVALID_VERSION;
    }
    if (read_u16(source, 6u) != HERMAS2_JOURNAL_RECORD_SIZE) {
        return HERMAS2_JOURNAL_INVALID_SIZE;
    }
    if (read_u32(source, 12u) != 0u ||
        read_u16(source, 50u) != 0u) {
        return HERMAS2_JOURNAL_NONZERO_RESERVED;
    }
    if (read_u32(source, 60u) != checksum(source, 60u)) {
        return HERMAS2_JOURNAL_CHECKSUM_MISMATCH;
    }
    hermas2_journal_record decoded = {
        .kind = (hermas2_journal_kind)read_u16(source, 8u),
    ...
    hermas2_journal_result valid = validate_record(&decoded);
    if (valid != HERMAS2_JOURNAL_OK) {
        return valid;
    }
    *record = decoded;
...
static scan_execution *find_scan_execution(
    ...
    for (size_t index = 0u;
         index < HERMAS2_JOURNAL_MAX_INTERRUPTED; ++index) {
...
static bool route_matches(
    const hermas2_journal_interrupted *execution,
    const hermas2_journal_record *record) {
...
static hermas2_journal_result scan_transition(
    scan_execution *executions,
    const hermas2_journal_record *record) {
    ...
    if (record->kind == HERMAS2_JOURNAL_EXECUTION_STARTED) {
        if (execution != NULL) {
            return HERMAS2_JOURNAL_INVALID_TRANSITION;
        }
        for (size_t index = 0u;
             index < HERMAS2_JOURNAL_MAX_INTERRUPTED; ++index) {
            if (!executions[index].active) {
        ...
        return HERMAS2_JOURNAL_CAPACITY_EXHAUSTED;
    }
    if (execution == NULL ||
    ...
    if (record->kind == HERMAS2_JOURNAL_DELIVERY_PREPARED) {
    ...
    if (record->kind == HERMAS2_JOURNAL_DELIVERY_SENT) {
    ...
    if (record->kind >= HERMAS2_JOURNAL_ACTION_SUCCEEDED &&
        ...
        if (execution->delivery == SCAN_DELIVERY_NONE ||
            !route_matches(&execution->value, record) ||
            (record->kind == HERMAS2_JOURNAL_ACTION_SUCCEEDED &&
             execution->delivery != SCAN_DELIVERY_SENT) ||
            (record->kind == HERMAS2_JOURNAL_ACTION_FAILED &&
             record->outcome == HERMAS2_OUTCOME_APP_ERROR &&
             execution->delivery != SCAN_DELIVERY_SENT) ||
            (record->kind == HERMAS2_JOURNAL_ACTION_FAILED &&
             record->outcome == HERMAS2_OUTCOME_NOT_SENT &&
    ...
    if (record->kind == HERMAS2_JOURNAL_EXECUTION_FINISHED) {
        if (execution->delivery != SCAN_DELIVERY_NONE ||
...
hermas2_journal_result hermas2_journal_scan(
    ...
    if ((bytes == NULL && byte_count != 0u) || summary == NULL ||
        byte_count % HERMAS2_JOURNAL_RECORD_SIZE != 0u) {
        return HERMAS2_JOURNAL_INVALID_ARGUMENT;
    }
    scan_execution executions[HERMAS2_JOURNAL_MAX_INTERRUPTED];
    memset(executions, 0, sizeof(executions));
    hermas2_journal_summary result;
    ...
    for (size_t offset = 0u; offset < byte_count;
         offset += HERMAS2_JOURNAL_RECORD_SIZE) {
        hermas2_journal_record record;
        hermas2_journal_result decoded = hermas2_journal_decode(
            bytes + offset, HERMAS2_JOURNAL_RECORD_SIZE, &record);
        if (decoded != HERMAS2_JOURNAL_OK) {
            return decoded;
        }
        ...
        hermas2_journal_result transitioned =
            scan_transition(executions, &record);
        if (transitioned != HERMAS2_JOURNAL_OK) {
            return transitioned;
        }
        if (visitor != NULL) {
            hermas2_journal_result visited =
                visitor(visitor_context, &record);
            if (visited != HERMAS2_JOURNAL_OK) {
                return visited;
            }
        }
        ++result.record_count;
...
hermas2_journal_result hermas2_journal_writer_append(
    ...
    hermas2_journal_result result =
        hermas2_journal_encode(&record, encoded, sizeof(encoded));
