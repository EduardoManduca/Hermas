...
static bool known_kind(uint16_t kind) {
    return kind >= HERMAS2_FRAME_REGISTER_APP &&
           kind <= HERMAS2_FRAME_EXECUTION_RESULT;
}

static bool valid_outcome(uint16_t outcome) {
    return outcome <= HERMAS2_OUTCOME_PROTOCOL_ERROR;
}
    ...
    switch (frame->kind) {
        ...
        case HERMAS2_FRAME_INVOKE:
            return routed && frame->app_id != 0u && frame->action_id != 0u &&
                       ...
                       ? HERMAS2_PROTOCOL_OK
                       : HERMAS2_PROTOCOL_INVALID_IDENTIFIER;
        case HERMAS2_FRAME_RESULT:
            return routed && frame->app_id != 0u && frame->action_id != 0u &&
                           frame->source_type != 0u &&
                           frame->destination_type != 0u &&
                           (frame->outcome == HERMAS2_OUTCOME_SUCCESS ||
                            frame->outcome == HERMAS2_OUTCOME_APP_ERROR)
                       ? HERMAS2_PROTOCOL_OK
                       : HERMAS2_PROTOCOL_INVALID_OUTCOME;
        case HERMAS2_FRAME_PROTOCOL_ERROR:
            return frame->outcome == HERMAS2_OUTCOME_PROTOCOL_ERROR &&
                           frame->payload_length == 0u
                       ? HERMAS2_PROTOCOL_OK
                       : HERMAS2_PROTOCOL_INVALID_OUTCOME;
        case HERMAS2_FRAME_EXECUTE:
            return frame->execution_id != 0u && frame->request_id == 0u &&
                       ...
                       ? HERMAS2_PROTOCOL_OK
                       : HERMAS2_PROTOCOL_INVALID_IDENTIFIER;
        case HERMAS2_FRAME_EXECUTION_RESULT:
            return frame->execution_id != 0u && frame->request_id == 0u &&
                           frame->app_id == 0u && frame->action_id == 0u &&
                           (((frame->outcome == HERMAS2_OUTCOME_SUCCESS ||
                              frame->outcome == HERMAS2_OUTCOME_APP_ERROR) &&
                             frame->source_type != 0u &&
                             frame->destination_type != 0u) ||
                            ((frame->outcome == HERMAS2_OUTCOME_NOT_SENT ||
                              frame->outcome == HERMAS2_OUTCOME_UNKNOWN) &&
                             frame->source_type == 0u &&
                             frame->destination_type == 0u &&
                             frame->payload_length == 0u))
                       ? HERMAS2_PROTOCOL_OK
                       : HERMAS2_PROTOCOL_INVALID_OUTCOME;
        default:
            return HERMAS2_PROTOCOL_INVALID_KIND;
    }
}
...
hermas2_protocol_result hermas2_protocol_encode(
    ...
    if (frame == NULL || destination == NULL || encoded_size == NULL) {
        return HERMAS2_PROTOCOL_BUFFER_TOO_SMALL;
    }
    *encoded_size = 0u;
    ...
    if (!valid_outcome(frame->outcome)) {
        return HERMAS2_PROTOCOL_INVALID_OUTCOME;
    }
    if (frame->payload_length > HERMAS2_PROTOCOL_MAX_PAYLOAD_SIZE) {
        return HERMAS2_PROTOCOL_FRAME_TOO_LARGE;
    }
    if (frame->payload_length != 0u && frame->payload == NULL) {
        return HERMAS2_PROTOCOL_INVALID_PAYLOAD;
    }
    hermas2_protocol_result semantic = validate_semantics(frame);
    if (semantic != HERMAS2_PROTOCOL_OK) {
        return semantic;
    }
    size_t total = HERMAS2_PROTOCOL_HEADER_SIZE + (size_t)frame->payload_length;
    ...
    if (frame->payload_length != 0u) {
        memcpy(destination + HERMAS2_PROTOCOL_HEADER_SIZE,
               frame->payload, frame->payload_length);
    }
    *encoded_size = total;
    return HERMAS2_PROTOCOL_OK;
}
...
hermas2_protocol_result hermas2_protocol_decode(
    ...
    if (packet == NULL || frame == NULL ||
        packet_size < HERMAS2_PROTOCOL_HEADER_SIZE) {
        return HERMAS2_PROTOCOL_TRUNCATED;
    }
    if (packet_size > HERMAS2_PROTOCOL_MAX_PACKET_SIZE) {
        return HERMAS2_PROTOCOL_FRAME_TOO_LARGE;
    }
    ...
    if (read_u32(packet, 8u) != 0u) {
        return HERMAS2_PROTOCOL_INVALID_FLAGS;
    }
    if (read_u32(packet, 12u) != HERMAS2_PROTOCOL_HEADER_SIZE ||
        read_u16(packet, 42u) != 0u) {
        return HERMAS2_PROTOCOL_NONZERO_RESERVED;
    }
    uint32_t payload_length = read_u32(packet, 44u);
    if ((size_t)payload_length != packet_size - HERMAS2_PROTOCOL_HEADER_SIZE) {
        return HERMAS2_PROTOCOL_LENGTH_MISMATCH;
    }
    ...
    hermas2_frame decoded = {
        ...
        .outcome = outcome,
        .payload = packet + HERMAS2_PROTOCOL_HEADER_SIZE,
        .payload_length = payload_length
    };
    hermas2_protocol_result semantic = validate_semantics(&decoded);
    if (semantic != HERMAS2_PROTOCOL_OK) {
        return semantic;
    }
    *frame = decoded;
