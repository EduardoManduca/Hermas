...
static bool key_equal(
    hermas2_result_key left,
    hermas2_result_key right) {
...
static bool record_valid(const hermas2_result_record *record) {
...
hermas2_result_store_result hermas2_result_encode(
    ...
    size_t size = HERMAS2_RESULT_HEADER_SIZE +
                  record->value_length;
    if (destination_size < size) {
        return HERMAS2_RESULT_STORE_BUFFER_TOO_SMALL;
    }
    memset(destination, 0, HERMAS2_RESULT_HEADER_SIZE);
    memcpy(destination, "H2RS", 4u);
    put_u16(destination, 4u, HERMAS2_RESULT_VERSION);
    put_u16(destination, 6u, HERMAS2_RESULT_HEADER_SIZE);
    ...
    if (record->value_length != 0u) {
        memcpy(
            destination + HERMAS2_RESULT_HEADER_SIZE,
            record->value, record->value_length);
    }
    put_u32(destination, 56u, checksum_parts(
        destination, destination + HERMAS2_RESULT_HEADER_SIZE,
        record->value_length));
    *encoded_size = size;
...
hermas2_result_store_result hermas2_result_decode(
    ...
    if (source_size < HERMAS2_RESULT_HEADER_SIZE) {
        return HERMAS2_RESULT_STORE_BUFFER_TOO_SMALL;
    }
    uint32_t size = get_u32(source, 8u);
    uint32_t value_length = get_u32(source, 12u);
    if (memcmp(source, "H2RS", 4u) != 0 ||
        get_u16(source, 4u) != HERMAS2_RESULT_VERSION ||
        get_u16(source, 6u) != HERMAS2_RESULT_HEADER_SIZE ||
        size != HERMAS2_RESULT_HEADER_SIZE + value_length ||
        size > source_size || get_u32(source, 36u) != 0u ||
        get_u16(source, 54u) != 0u ||
        get_u32(source, 60u) != 0u ||
        value_length > HERMAS2_PROTOCOL_MAX_PAYLOAD_SIZE) {
        return HERMAS2_RESULT_STORE_INVALID_RECORD;
    }
    if (get_u32(source, 56u) != checksum_parts(
            source, source + HERMAS2_RESULT_HEADER_SIZE,
            value_length)) {
        return HERMAS2_RESULT_STORE_CHECKSUM_MISMATCH;
    }
    hermas2_result_record decoded = {
        ...
        .value = source + HERMAS2_RESULT_HEADER_SIZE,
...
hermas2_result_store_result hermas2_result_scan(
    ...
    hermas2_result_summary result = {.next_sequence = 1u};
    size_t offset = 0u;
    while (offset < byte_count) {
        hermas2_result_record record;
        size_t size = 0u;
        hermas2_result_store_result decoded =
            hermas2_result_decode(
                bytes + offset, byte_count - offset,
                &record, &size);
        if (decoded != HERMAS2_RESULT_STORE_OK) {
            return decoded;
        }
        if (record.sequence != result.next_sequence) {
            return HERMAS2_RESULT_STORE_INVALID_SEQUENCE;
        }
        size_t prior_offset = 0u;
        while (prior_offset < offset) {
            hermas2_result_record prior;
            size_t prior_size = 0u;
            hermas2_result_store_result prior_decoded =
                hermas2_result_decode(
                    bytes + prior_offset,
                    offset - prior_offset,
                    &prior, &prior_size);
            if (prior_decoded != HERMAS2_RESULT_STORE_OK) {
                return prior_decoded;
            }
            if (key_equal(prior.key, record.key)) {
                return HERMAS2_RESULT_STORE_DUPLICATE_RESULT;
            }
            prior_offset += prior_size;
        }
        if (visitor != NULL) {
            hermas2_result_store_result visited =
                visitor(visitor_context, &record);
            if (visited != HERMAS2_RESULT_STORE_OK) {
                return visited;
            }
        }
        ++result.record_count;
...
hermas2_result_store_result hermas2_result_find(
    ...
    *found = 0;
    size_t offset = 0u;
    while (offset < byte_count) {
        hermas2_result_record candidate;
        size_t size = 0u;
        hermas2_result_store_result decoded =
            hermas2_result_decode(
                bytes + offset, byte_count - offset,
...
hermas2_result_store_result hermas2_result_writer_append(
    ...
    size_t size = 0u;
    hermas2_result_store_result encoded =
        hermas2_result_encode(
            &record, scratch, scratch_capacity, &size);
    if (encoded != HERMAS2_RESULT_STORE_OK) {
        return encoded;
    }
    hermas2_result_store_result written =
        writer->write(writer->context, scratch, size);
    if (written != HERMAS2_RESULT_STORE_OK) {
        return written;
    }
