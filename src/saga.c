...
static bool load_steps(hermas_saga_execution *execution) {
    uint16_t regions = read_u16(
        execution->image, HERMAS_IMAGE_HEADER_REGION_COUNT_OFFSET);
    size_t base = read_u32(
        execution->image, HERMAS_IMAGE_HEADER_REGIONS_OFFSET);
    for (uint16_t index = 0u; index < regions; ++index) {
        ...
        if (ordinal == 0u || ordinal > HERMAS_SAGA_MAX_STEPS ||
            index + 1u >= regions) {
            return false;
        }
        size_t outcome = offset + HERMAS_IMAGE_REGION_RECORD_SIZE;
        if (execution->image[outcome] != 4u ||
            read_u16(execution->image, outcome + 2u) !=
                read_u16(execution->image, offset + 2u)) {
            return false;
        }
        hermas_saga_step *step = &execution->steps[ordinal - 1u];
...
static hermas_saga_step *find_step(
...
static bool forward_route_matches(
    ...
    size_t nodes = read_u32(
        execution->image, HERMAS_IMAGE_HEADER_NODES_OFFSET);
    size_t node = nodes +
...
static hermas_saga_result validate_token(
    const hermas_saga_execution *execution,
    const hermas_saga_step *step,
    uint8_t *buffer,
    size_t capacity,
    ...
    int found = 0;
    hermas_compensation_result found_result =
    ...
    if (found_result == HERMAS_COMPENSATION_DUPLICATE_TOKEN) {
        return HERMAS_SAGA_DUPLICATE_TOKEN;
    }
    if (found_result == HERMAS_COMPENSATION_BUFFER_TOO_SMALL) {
        return HERMAS_SAGA_BUFFER_TOO_SMALL;
    }
    if (found_result != HERMAS_COMPENSATION_OK) {
        return HERMAS_SAGA_INVALID_TOKEN;
    }
    if (found == 0) {
        return HERMAS_SAGA_MISSING_TOKEN;
    }
    if (record->compensation_app_id != step->compensation_app_id ||
        ...
        hermas_image_validate_value(
            ...
            record->token_length) != HERMAS_IMAGE_OK) {
        return HERMAS_SAGA_INVALID_TOKEN;
    }
    return HERMAS_SAGA_OK;
}
...
hermas_saga_result hermas_saga_recover(
    ...
    if (execution == NULL || image == NULL ||
        ...
        return HERMAS_SAGA_INVALID_ARGUMENT;
    }
    if (hermas_image_validate(image, image_size, NULL) !=
        HERMAS_IMAGE_OK) {
        return HERMAS_SAGA_INVALID_IMAGE;
    }
    hermas_journal_summary summary;
    if (hermas_journal_scan(journal, journal_bytes, NULL, NULL,
                             &summary) != HERMAS_JOURNAL_OK) {
        return HERMAS_SAGA_INCONSISTENT_HISTORY;
    }
    hermas_compensation_summary token_summary;
    if (hermas_compensation_scan(tokens, token_bytes, NULL, NULL,
                                  &token_summary) !=
    ...
    for (size_t index = 0u; index < record_count; ++index) {
        ...
        if (record.kind == HERMAS_JOURNAL_ACTION_SUCCEEDED) {
            ...
            ++execution->remaining;
        } else if (record.kind == HERMAS_JOURNAL_ACTION_FAILED ||
    ...
    if (execution->original_outcome == HERMAS_OUTCOME_UNKNOWN) {
        execution->state = HERMAS_SAGA_BLOCKED;
        return HERMAS_SAGA_UNSAFE_HISTORY;
    }
    if (execution->original_outcome == HERMAS_OUTCOME_SUCCESS) {
        execution->remaining = 0u;
        execution->state = HERMAS_SAGA_COMPLETE;
        return HERMAS_SAGA_OK;
    }
    if (execution->original_outcome != HERMAS_OUTCOME_APP_ERROR &&
        execution->original_outcome != HERMAS_OUTCOME_NOT_SENT) {
        return HERMAS_SAGA_INCONSISTENT_HISTORY;
    }
    execution->next_request_id = largest_request + 1u;
    for (uint8_t index = 0u; index < execution->remaining; ++index) {
        uint8_t scratch[HERMAS_PROTOCOL_MAX_PAYLOAD_SIZE];
        hermas_compensation_record token;
        hermas_saga_result valid = validate_token(
            execution, &execution->steps[index], scratch,
            sizeof(scratch), &token);
        if (valid != HERMAS_SAGA_OK) {
            return valid;
        }
    }
...
hermas_saga_result hermas_saga_begin_live(
    ...
    if (execution == NULL || image == NULL || execution_id == 0u ||
        ...
        completed_steps == 0u ||
        completed_steps > HERMAS_SAGA_MAX_STEPS ||
        next_request_id == 0u || next_request_id == UINT64_MAX ||
        token_lookup == NULL ||
        (original_outcome != HERMAS_OUTCOME_APP_ERROR &&
         original_outcome != HERMAS_OUTCOME_NOT_SENT)) {
        return HERMAS_SAGA_INVALID_ARGUMENT;
    }
    if (hermas_image_validate(image, image_size, NULL) !=
        HERMAS_IMAGE_OK) {
...
hermas_saga_result hermas_saga_prepare(
    ...
    if (execution->state != HERMAS_SAGA_READY ||
        execution->remaining == 0u) {
        return HERMAS_SAGA_INVALID_STATE;
    }
    if (execution->next_request_id == 0u ||
        execution->next_request_id == UINT64_MAX) {
        return HERMAS_SAGA_REQUEST_ID_EXHAUSTED;
    }
    hermas_saga_step *step =
    ...
    *invocation = (hermas_frame){
    ...
    execution->state = HERMAS_SAGA_PREPARED;
    return HERMAS_SAGA_OK;
}
...
hermas_saga_result hermas_saga_reconcile(
    ...
    bool started = false;
    bool finished = false;
    uint16_t finished_outcome = HERMAS_OUTCOME_NONE;
    uint16_t initial_ordinal = 0u;
    uint64_t largest_request = execution->next_request_id - 1u;
    size_t count =
        saga_log_bytes / HERMAS_SAGA_LOG_RECORD_SIZE;
    for (size_t index = 0u; index < count; ++index) {
        hermas_saga_log_record record;
        if (hermas_saga_log_decode(
                saga_log + index * HERMAS_SAGA_LOG_RECORD_SIZE,
                HERMAS_SAGA_LOG_RECORD_SIZE, &record) !=
            HERMAS_SAGA_LOG_OK) {
            return HERMAS_SAGA_INCONSISTENT_HISTORY;
        }
    ...
    if (finished) {
        execution->remaining =
            finished_outcome == HERMAS_OUTCOME_SUCCESS
                ? 0u
                : execution->remaining;
    ...
    const hermas_saga_log_active *active = NULL;
    ...
    if (active->terminal_outcome != HERMAS_OUTCOME_NONE) {
        execution->compensation_outcome =
...
hermas_saga_result hermas_saga_mark_sent(
    ...
    execution->state = HERMAS_SAGA_SENT;
    return HERMAS_SAGA_OK;
}
...
hermas_saga_result hermas_saga_mark_not_sent(
    ...
    execution->compensation_outcome = HERMAS_OUTCOME_NOT_SENT;
    execution->state = HERMAS_SAGA_BLOCKED;
    return HERMAS_SAGA_OK;
}
...
hermas_saga_result hermas_saga_mark_unknown(
    ...
    execution->compensation_outcome = HERMAS_OUTCOME_UNKNOWN;
    execution->state = HERMAS_SAGA_BLOCKED;
    return HERMAS_SAGA_OK;
}
...
hermas_saga_result hermas_saga_accept_result(
    ...
    if (execution->state != HERMAS_SAGA_SENT ||
        execution->remaining == 0u) {
        return HERMAS_SAGA_INVALID_STATE;
    }
    const hermas_saga_step *step =
    ...
    if (result->kind != HERMAS_FRAME_RESULT ||
        ...
        return HERMAS_SAGA_UNEXPECTED_RESULT;
    }
    ...
    if (result->outcome == HERMAS_OUTCOME_APP_ERROR) {
        execution->state = HERMAS_SAGA_BLOCKED;
        return HERMAS_SAGA_OK;
    }
    --execution->remaining;
...
static hermas_saga_log_record driver_record(
    const hermas_saga_driver *driver,
    ...
    } else if (kind == HERMAS_SAGA_LOG_STARTED) {
        record.ordinal = driver->execution.remaining;
    }
    return record;
}

static hermas_saga_result append_driver_record(
    hermas_saga_driver *driver,
    hermas_saga_log_kind kind,
    uint16_t outcome,
    bool action) {
    return hermas_saga_log_writer_append(
               driver->log,
...
hermas_saga_result hermas_saga_driver_begin(
    ...
    if (driver->execution.state == HERMAS_SAGA_COMPLETE ||
        driver->started != 0u) {
        return HERMAS_SAGA_OK;
    }
    hermas_saga_result appended = append_driver_record(
        driver, HERMAS_SAGA_LOG_STARTED,
        driver->execution.original_outcome, false);
    if (appended == HERMAS_SAGA_OK) {
        driver->started = 1u;
    }
    return appended;
}
...
hermas_saga_result hermas_saga_driver_prepare(
    ...
    hermas_saga_result prepared = hermas_saga_prepare(
        &driver->execution, token_buffer, token_capacity, invocation);
    if (prepared != HERMAS_SAGA_OK) {
        return prepared;
    }
    return append_driver_record(
        driver, HERMAS_SAGA_LOG_DELIVERY_PREPARED,
...
hermas_saga_result hermas_saga_driver_mark_sent(
    ...
    hermas_saga_result changed =
        hermas_saga_mark_sent(&driver->execution);
    if (changed != HERMAS_SAGA_OK) {
        return changed;
    }
    return append_driver_record(
        driver, HERMAS_SAGA_LOG_DELIVERY_SENT,
        HERMAS_OUTCOME_NONE, true);
}

static hermas_saga_result finish_driver(
    hermas_saga_driver *driver,
    hermas_saga_log_kind kind,
    uint16_t outcome) {
    hermas_saga_result step =
        append_driver_record(driver, kind, outcome, true);
    if (step != HERMAS_SAGA_OK) {
        return step;
    }
    return append_driver_record(
        driver, HERMAS_SAGA_LOG_FINISHED, outcome, false);
}
...
hermas_saga_result hermas_saga_driver_mark_not_sent(
    ...
    hermas_saga_result changed =
        hermas_saga_mark_not_sent(&driver->execution);
    if (changed != HERMAS_SAGA_OK) {
        return changed;
    }
    return finish_driver(
        driver, HERMAS_SAGA_LOG_STEP_FAILED,
...
hermas_saga_result hermas_saga_driver_mark_unknown(
    ...
    hermas_saga_result changed =
        hermas_saga_mark_unknown(&driver->execution);
    if (changed != HERMAS_SAGA_OK) {
        return changed;
    }
    return finish_driver(
        driver, HERMAS_SAGA_LOG_STEP_UNKNOWN,
        HERMAS_OUTCOME_UNKNOWN);
}
...
hermas_saga_result hermas_saga_driver_accept_result(
    ...
    hermas_saga_log_record durable = driver_record(
        driver,
        outcome == HERMAS_OUTCOME_SUCCESS
            ? HERMAS_SAGA_LOG_STEP_SUCCEEDED
            : HERMAS_SAGA_LOG_STEP_FAILED,
        outcome, true);
    hermas_saga_result accepted =
        hermas_saga_accept_result(&driver->execution, result);
    if (accepted != HERMAS_SAGA_OK) {
        return accepted;
    }
    if (hermas_saga_log_writer_append(
            driver->log, durable) != HERMAS_SAGA_LOG_OK) {
        return HERMAS_SAGA_LOG_ERROR;
    }
    if (driver->execution.state == HERMAS_SAGA_COMPLETE ||
        driver->execution.state == HERMAS_SAGA_BLOCKED) {
        return append_driver_record(
            driver, HERMAS_SAGA_LOG_FINISHED,
            outcome == HERMAS_OUTCOME_SUCCESS
