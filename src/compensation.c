...
static bool key_equal(
    hermas_compensation_key left,
    hermas_compensation_key right) {
...
static bool record_valid(const hermas_compensation_record *record) {
    return record->sequence != 0u &&
           ...
           record->token_length <= HERMAS_PROTOCOL_MAX_PAYLOAD_SIZE &&
...
hermas_compensation_result hermas_compensation_encode(
    ...
    size_t size = HERMAS_COMPENSATION_HEADER_SIZE +
                  record->token_length;
    if (destination_size < size) {
        return HERMAS_COMPENSATION_BUFFER_TOO_SMALL;
    }
    memset(destination, 0, HERMAS_COMPENSATION_HEADER_SIZE);
    memcpy(destination, "HCT1", 4u);
    put_u16(destination, 4u, HERMAS_COMPENSATION_VERSION);
    put_u16(destination, 6u, HERMAS_COMPENSATION_HEADER_SIZE);
    ...
    if (record->token_length != 0u) {
        memcpy(destination + HERMAS_COMPENSATION_HEADER_SIZE,
               record->token, record->token_length);
    }
    put_u32(destination, 68u, checksum_parts(
        destination,
        destination + HERMAS_COMPENSATION_HEADER_SIZE,
        record->token_length));
    *encoded_size = size;
...
hermas_compensation_result hermas_compensation_decode(
    ...
    if (source_size < HERMAS_COMPENSATION_HEADER_SIZE) {
        return HERMAS_COMPENSATION_BUFFER_TOO_SMALL;
    }
    uint32_t size = get_u32(source, 8u);
    uint32_t token_length = get_u32(source, 12u);
    if (memcmp(source, "HCT1", 4u) != 0 ||
        get_u16(source, 4u) != HERMAS_COMPENSATION_VERSION ||
        get_u16(source, 6u) != HERMAS_COMPENSATION_HEADER_SIZE ||
        size != HERMAS_COMPENSATION_HEADER_SIZE + token_length ||
        size > source_size || get_u16(source, 54u) != 0u ||
        get_u32(source, 64u) != 0u ||
        token_length > HERMAS_PROTOCOL_MAX_PAYLOAD_SIZE) {
        return HERMAS_COMPENSATION_INVALID_RECORD;
    }
    if (get_u32(source, 68u) != checksum_parts(
            source, source + HERMAS_COMPENSATION_HEADER_SIZE,
            token_length)) {
        return HERMAS_COMPENSATION_CHECKSUM_MISMATCH;
    }
    hermas_compensation_record decoded = {
        ...
        .token = source + HERMAS_COMPENSATION_HEADER_SIZE,
...
hermas_compensation_result hermas_compensation_scan(
    ...
    hermas_compensation_summary result = {
        .next_sequence = 1u
    };
    size_t offset = 0u;
    while (offset < byte_count) {
        hermas_compensation_record record;
        size_t size = 0u;
        hermas_compensation_result decoded =
            hermas_compensation_decode(
                bytes + offset, byte_count - offset, &record, &size);
        if (decoded != HERMAS_COMPENSATION_OK) {
            return decoded;
        }
        if (record.sequence != result.next_sequence) {
            return HERMAS_COMPENSATION_INVALID_SEQUENCE;
        }
        if (visitor != NULL) {
            hermas_compensation_result visited =
                visitor(visitor_context, &record);
            if (visited != HERMAS_COMPENSATION_OK) {
                return visited;
            }
        }
        ++result.record_count;
...
hermas_compensation_result hermas_compensation_find(
    ...
    *found = 0;
    size_t offset = 0u;
    while (offset < byte_count) {
        hermas_compensation_record candidate;
        size_t size = 0u;
        hermas_compensation_result decoded =
            hermas_compensation_decode(
        ...
        if (key_equal(candidate.key, key)) {
            if (*found != 0) {
                return HERMAS_COMPENSATION_DUPLICATE_TOKEN;
            }
...
hermas_compensation_result hermas_compensation_writer_init(
    ...
    *writer = (hermas_compensation_writer){
        .write = write,
        .context = context,
...
hermas_compensation_result hermas_compensation_writer_append(
    ...
    size_t size = 0u;
    hermas_compensation_result encoded =
        hermas_compensation_encode(
            &record, scratch, scratch_capacity, &size);
    if (encoded != HERMAS_COMPENSATION_OK) {
        return encoded;
    }
    hermas_compensation_result written =
        writer->write(writer->context, scratch, size);
    if (written != HERMAS_COMPENSATION_OK) {
        return written;
    }
