...
#define HERMAS_HEADER_APPS_OFFSET 40u
#define HERMAS_ACTION_CONTRACT_RECORD_SIZE 36u
...
static hermas_daemon_result receive_packet(
    int descriptor,
    uint8_t *packet,
    size_t capacity,
    size_t *packet_size) {
    ...
    if (received <= 0) {
        return HERMAS_DAEMON_RECEIVE_ERROR;
    }
    if ((message.msg_flags & MSG_TRUNC) != 0 ||
        (size_t)received > capacity) {
        return HERMAS_DAEMON_TRUNCATED_PACKET;
    }
    *packet_size = (size_t)received;
    return HERMAS_DAEMON_OK;
}
...
hermas_daemon_result hermas_daemon_registry_init(
    ...
    if (registry == NULL || image == NULL) {
        return HERMAS_DAEMON_INVALID_ARGUMENT;
    }
    hermas_image_summary summary;
    if (hermas_image_validate(image, image_size, &summary) !=
            HERMAS_IMAGE_OK ||
        summary.action_contract_count > HERMAS_DAEMON_MAX_ACTIONS) {
        return HERMAS_DAEMON_INVALID_IMAGE;
    }
    hermas_daemon_registry fresh;
    memset(&fresh, 0, sizeof(fresh));
    fresh.action_count = summary.action_contract_count;
    size_t apps_offset = read_u32(image, HERMAS_HEADER_APPS_OFFSET);
    for (size_t index = 0u; index < fresh.action_count; ++index) {
        size_t offset =
            apps_offset + index * HERMAS_ACTION_CONTRACT_RECORD_SIZE;
...
hermas_daemon_result hermas_daemon_registry_accept(
    ...
    if (registry == NULL || listener < 0 || packet_buffer == NULL ||
        packet_capacity < HERMAS_PROTOCOL_HEADER_SIZE + 32u) {
        return HERMAS_DAEMON_INVALID_ARGUMENT;
    }
    int connection;
    do {
    ...
    if (connection < 0) {
        return HERMAS_DAEMON_ACCEPT_ERROR;
    }
    size_t packet_size = 0u;
    hermas_daemon_result received =
        receive_packet(connection, packet_buffer, packet_capacity,
                       &packet_size);
    if (received != HERMAS_DAEMON_OK) {
        close(connection);
        return received;
    }
    hermas_frame registration;
    if (hermas_protocol_decode(packet_buffer, packet_size, &registration) !=
            HERMAS_PROTOCOL_OK ||
        registration.kind != HERMAS_FRAME_REGISTER_APP) {
        close(connection);
        return HERMAS_DAEMON_PROTOCOL_ERROR;
    }
    hermas_daemon_action *slot = NULL;
    ...
    if (slot == NULL) {
        close(connection);
        return app_expected ? HERMAS_DAEMON_CONTRACT_MISMATCH
                            : HERMAS_DAEMON_UNEXPECTED_APP;
    }
    if (slot->file_descriptor >= 0) {
        close(connection);
        return HERMAS_DAEMON_DUPLICATE_APP;
    }
...
int hermas_daemon_registry_find_action(
    ...
    for (size_t index = 0u; index < registry->action_count; ++index) {
        const hermas_daemon_action *slot = &registry->actions[index];
