# Hermas Action-Graph Architecture

## 1. Status of This Document

This document explains the graph semantics of the independent Hermas
experiment. The [Master Architecture Plan](00_HERMAS_MASTER_PLAN.md) governs
implementation order and component boundaries.

It retains proven Hermas principles only where they strengthen one central
model:

> An HScript workflow is a statically verified, bounded action graph.

HScript is the human-readable notation. The action graph is the meaning, and
...
The compiler translates readable HScript into this graph. The daemon executes
the graph without interpreting source-language syntax.

## 3. Clean-Slate Position

Hermas does not preserve the original HScript grammar, workflow image, wire
frames, or C ABI by default. The legacy prototype is used to
...
Values have HSchema types. Bindings are immutable.
...
Apps may return HSchema variants. `match` selects subsequent Actions by case:
...
Internally, each HScript scope can be represented as a typed coordination
context:
...
The HScript surface does not expose a generic `context` facility. Instead, it
...
`each` operates only on a statically bounded HSchema list:
...
One readable HScript expression may deterministically construct several
...
HScript may offer concise propagation:
...
daemon. No separate HScript interpretation is permitted.
...
HScript abstract syntax tree.

## 12. Runtime Execution

`hermasd` executes a loaded graph using fixed tables and bounded state:
...
The daemon does not parse HScript, resolve names dynamically, calculate
...
Hermas may change language spelling, graph encoding, protocol, and ABI when
doing so improves the graph model. It does not need to compile legacy
prototype files or run legacy applications.
...
2. Implement the Rust graph kernel before parsing HScript.
3. Specify HSchema Action ports.
4. Compile sequential HScript expressions into inspectable graphs.
...
The architecture succeeds if HScript remains easy to read while every
program has one precise, inspectable graph interpretation.

The intended relationship is:

```text
HSchema defines the typed ports exposed by Action nodes.
HScript constructs readable graph expressions.
