...
fn bounded_each_lowers_to_a_typed_ordered_collection_region() {
    let catalog = catalog();
    let graph = compile_hscript(&catalog, "batch.hscript2", WORKFLOW).unwrap();
    assert_eq!(
...
fn bounded_each_rejects_invalid_concurrency_and_collect_shape() {
    ...
    let error = compile_hscript(&catalog, "excessive.hscript2", &excessive).unwrap_err();
    ...
    let error = compile_hscript(&catalog, "small.hscript2", &small).unwrap_err();
...
fn bounded_each_initial_template_is_exactly_one_action() {
    ...
    let error = compile_hscript(&catalog, "two-actions.hscript2", &two_actions).unwrap_err();
    assert!(
