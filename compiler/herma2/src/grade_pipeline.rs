...
pub fn declare_grade_list(catalog: &mut Catalog) -> Result<GradeListContract, CatalogError> {
    let contract = compile_schema(catalog, "grade-list.hschema2", GRADE_LIST_SCHEMA)
...
pub fn declare_mean_calculator(
    ...
    let contract = compile_schema(catalog, "mean-calculator.hschema2", MEAN_CALCULATOR_SCHEMA)
        .map_err(schema_catalog_error)?;
...
pub fn declare_printer(catalog: &mut Catalog) -> Result<PrinterContract, CatalogError> {
    let contract = compile_schema(catalog, "printer.hschema2", PRINTER_SCHEMA)
        .map_err(schema_catalog_error)?;
...
impl fmt::Display for GradePipelineError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::HScript(error) => write!(formatter, "HScript2: {error}"),
...
pub fn build_grade_pipeline() -> Result<(Catalog, VerifiedGraph), GradePipelineError> {
    ...
    let verified = compile_hscript(&catalog, "grade-pipeline.hscript2", GRADE_PIPELINE_SCRIPT)?;
    Ok((catalog, verified))
}
