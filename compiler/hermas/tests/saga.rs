...
fn compile_fixture() -> (Catalog, hermas::VerifiedGraph) {
    let mut catalog = Catalog::new();
    compile_schema(
        &mut catalog,
        "payment.hschema",
        include_str!("../../../apps/saga/payment.hschema"),
    )
    .unwrap();
    compile_schema(
        &mut catalog,
        "shipping.hschema",
        include_str!("../../../apps/saga/shipping.hschema"),
    )
    .unwrap();
    let graph = compile_hscript(
        &catalog,
        "fulfill.hscript",
        include_str!("../../../apps/saga/fulfill.hscript"),
    )
    .unwrap();
    (catalog, graph)
}
...
"#;
    let error = compile_hscript(&catalog, "invalid.hscript", irreversible).unwrap_err();
...
"#;
    let error = compile_hscript(&catalog, "parallel.hscript", parallel).unwrap_err();
