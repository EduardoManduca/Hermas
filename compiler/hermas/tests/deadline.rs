...
use hermas::{Catalog, compile_hscript, decode_graph_image, encode_graph_image};
...
fn root_within_lowers_to_one_deadline_region() {
    let catalog = catalog();
    let graph = compile_hscript(&catalog, "timed.hscript", WITHIN).unwrap();
    assert_eq!(
...
fn root_within_requires_a_nonzero_bounded_duration() {
    ...
    let error = compile_hscript(&catalog, "zero.hscript", &zero).unwrap_err();
    assert_eq!(error.stage, "syntax");

    let unitless = WITHIN.replace("5s", "5");
    let error = compile_hscript(&catalog, "unitless.hscript", &unitless).unwrap_err();
