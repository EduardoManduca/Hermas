...
fn choice_catalog() -> (Catalog, hermas::SchemaContract) {
    let source = r#"
app decision
...
"#;
    ...
    let contract = compile_schema(&mut catalog, "decision.hschema", source).unwrap();
    (catalog, contract)
}
...
"#;
    let graph = compile_hscript(&catalog, "choose.hscript", source).unwrap();
...
"#;
    let diagnostic = compile_hscript(&catalog, "missing.hscript", missing).unwrap_err();
...
"#;
    let diagnostic = compile_hscript(&catalog, "duplicate.hscript", duplicate).unwrap_err();
