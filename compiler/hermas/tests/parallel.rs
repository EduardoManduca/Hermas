...
fn typed_all_lowers_to_bounded_fork_and_join() {
    ...
    let graph = compile_hscript(&catalog, "cooperate.hscript", WORKFLOW).unwrap();
...
fn typed_all_rejects_non_static_or_duplicate_branches() {
    ...
    let error = compile_hscript(&catalog, "small.hscript", &too_small).unwrap_err();
    assert_eq!(error.code, "all-branch-count");

    let duplicate = WORKFLOW.replace("right = item", "left = item");
    let error = compile_hscript(&catalog, "duplicate.hscript", &duplicate).unwrap_err();
...
fn typed_all_rejects_a_branch_that_escapes_the_shared_input() {
    ...
    let error = compile_hscript(&catalog, "wrong-source.hscript", &wrong).unwrap_err();
