...
{ return input |> printer/print() }";
    ...
    assert!(compile_hscript(&catalog, "hash.hscript", &with_hash).is_ok());
    assert!(compile_hscript(&catalog, "double-slash.hscript", &with_double_slash).is_err());
}
...
"#;
    let graph = compile_hscript(&catalog, "print.hscript", source).expect("typed workflow");
    assert!(graph.has_complete_source_map());
    assert!(
        graph
            .explain(&catalog)
...
fn one_module_compiles_independent_addressable_workflow_graphs() {
    let catalog = grade_catalog();
    let module = compile_hscript_module(&catalog, "operations.hscript", MULTI_WORKFLOW_MODULE)
    ...
    for name in ["calculate", "print"] {
        let graph =
            compile_hscript_workflow(&catalog, "operations.hscript", MULTI_WORKFLOW_MODULE, name)
...
fn single_workflow_api_requires_selection_for_a_module() {
    let catalog = grade_catalog();
    let error = compile_hscript(&catalog, "operations.hscript", MULTI_WORKFLOW_MODULE).unwrap_err();
...
fn module_rejects_duplicate_names_and_checks_every_workflow() {
    ...
    let error = compile_hscript_module(&catalog, "duplicate-module.hscript", &duplicate)
        .err()
    ...
    let error = compile_hscript_workflow(&catalog, "invalid-module.hscript", &invalid, "calculate")
        .unwrap_err();
    assert_eq!(error.stage, "resolution");
    assert_eq!(error.code, "unknown-action");
    ...
    let error = compile_hscript_module(&catalog, "unnamed-module.hscript", &unnamed)
        .err()
